...
begin
  ...
  if not (public.can_edit_newsroom() or public.can_review_newsroom()) then
  ...
  set updated_by = v_actor, updated_at = v_now
  where id = p_story_id;
...
begin
  ...
  if not public.can_review_newsroom() then
  ...
  if not found then
    raise exception 'editorial checklist is missing';
  end if;
  ...
  if p_decision = 'approved' then
    ...
    update public.stories
    set status = case when p_decision = 'changes_requested' then 'drafting'::public.story_status else 'fact_check'::public.story_status end,
